...
Mansion3TextBlock:
	TX_ASM
	CheckEvent EVENT_USED_REVEAL_GLASS
	jr nz, .asm_text_end ;if true, jump to end
	ld hl, RevealGlassText
	call PrintText ;regular text
	ld b, REVEAL_GLASS
	call IsItemInBag
	jr z, .asm_text_end ;if no reveal glass, jump to end
	ld hl, HaveGlassText
	call PrintText ;showing reveal glass text
	
	call GBFadeOutToBlack
	ld a, HS_MANSION_3_BLOCK
	...
	SetEvent EVENT_USED_REVEAL_GLASS ;set the event
.asm_text_end
	jp TextScriptEnd


RevealGlassText:
	TX_FAR _RevealGlassText
	db "@"

HaveGlassText:
	TX_FAR _HaveGlassText
	db "@"
